...
<article class="page-container">
    <form class="form form-horizontal" id="form-canteen">
                ...
                <span class="select-box">
                    <select name="school_id" class="select">
                        ...
                        @foreach ($school_list as $item) 
                            <option value="{{$item->id}}" {{$info->school_id == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
                        @endforeach
                    </select>
                </span>
            </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text"  value="{{$info->name}}" placeholder="食堂名称" id="name" name="name"  onblur="input_check(this)" autocomplete = 'off'>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-3 col-sm-3"><span class="c-red">*</span>抽成比例：</label>
            <div class="formControls col-xs-8 col-sm-9">
            <input type="text" class="input-text"  value="{{$info->cut_proportion}}" placeholder="抽成比例" id="cut_proportion" name="cut_proportion"  onblur="input_check(this)" autocomplete = 'off'>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-3 col-sm-3"><span class="c-red">*</span>账户名：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text"  value="{{$info->account}}" placeholder="账户名" id="account" name="account"  onblur="input_check(this)"  autocomplete = 'off'>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-3 col-sm-3"><span class="c-red">*</span>密码：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text"  value="{{$info->cleartext}}" placeholder="密码" id="cleartext" name="cleartext"  onblur="input_check(this)"  autocomplete = 'off'>
            </div>
        </div>
        <div class="row cl">
        ...
        <div class="row cl">
            <div class="col-xs-8 col-sm-9 col-xs-offset-3 col-sm-offset-3">
                <input class="btn btn-primary radius" type="button"   onclick="canteen_submit('/admin/canteen/update/{{$info->id}}')" value="&nbsp;&nbsp;提交&nbsp;&nbsp;">
