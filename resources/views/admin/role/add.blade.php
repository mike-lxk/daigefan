...
<form class="form form-horizontal" id="form-role">
	...
	<div class="row cl" style="margin-bottom:15px;margin-right:25px;">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="" id="name" name="name" onblur="input_check(this)"  autocomplete = 'off'>
		</div>
	</div>
	<div class="row cl" style="margin-bottom:15px;margin-right:25px;">
		<label class="form-label col-xs-4 col-sm-2" style="text-align:right;">备注：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="" placeholder="" id="depict" name="depict"  autocomplete = 'off'>
		</div>
	</div>
	<div class="row cl" style="margin-bottom:15px;margin-right:25px;">
		...
		<div class="formControls col-xs-8 col-sm-9">
            @foreach($list as $item)
					...
					<dl class="permission-list">
						<dt>
							<label>
								<input type="checkbox" value="{{$item->id}}" name="choose" id="user-Character-0">
								{{$item->name}}</label>
						</dt>
						<dd>
                            @foreach($list as $utils)
                                @if($utils->fid == $item->id)
									<label class="">
										<input type="checkbox" value="{{$utils->id}}" name="choose" id="user-Character-0-0-0">
										{{$utils->name}}
									</label>&nbsp;&nbsp;&nbsp;
                                @endif
                            @endforeach
	...
	<div class="row cl">
		<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
            <input class="btn btn-primary radius" type="button"  onclick="role_submit('/admin/role/insert')" value="&nbsp;&nbsp;提交&nbsp;&nbsp;">
